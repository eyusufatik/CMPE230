...
char* convert_complex_expr(char *expr, int *ret_type, int *rows, int *cols){
    ...
    if(match_binary_arithmetic_expr(expr)){
        ...
        *ret_type = out_type;
        ...
        char *ret = malloc(strlen(tokens.elements[0]) + 19 + strlen(tokens.elements[1]) + 12 + strlen(tokens.elements[2]) + 3);
        ...
        *cols = 1;
        ...
        vector tokens = tokenize(expr, " [,]");
        // {var_name}.elements[{i}]
        char *ret = malloc(strlen(tokens.elements[0]) + 19 + strlen(tokens.elements[1]) + 3);
        strcpy(ret, tokens.elements[0]);
        strcat(ret, ".elements[my_round(");
        strcat(ret, tokens.elements[1]);
        strcat(ret, ")]\0");
